...
<div class="row g-3 mb-4">
    ...
    <div class="col-6 col-md-4">
        <div class="card border-0 shadow-sm rounded-4 h-100 border-start border-warning border-4">
            <div class="card-body">
                <h6 class="text-muted fw-normal mb-2">Belum Ditempatkan</h6>
                <h3 class="mb-0 fw-bold text-warning">{{ number_format($stats['belum']) }}</h3>
            </div>
        </div>
    </div>
</div>
...
<div class="card shadow-sm border-0 rounded-4">
    ...
    <div class="card-body p-4">
        <form method="GET" action="{{ route('wms.inbound.putaway') }}" class="row g-2 mb-4 align-items-stretch">
            ...
            <div class="col-4 col-md-2 d-flex gap-2">
                ...
                <a href="{{ route('wms.inbound.putaway') }}" class="btn btn-outline-secondary" title="Reset filter"><i class="bi bi-arrow-counterclockwise"></i></a>
            </div>
        </form>
                ...
                <tbody>
                    @forelse($documents as $doc)
                        @php
                            ...
                            $sudah = $doc->details_placed_count;
                            // Dokumen yang sudah tersentuh dibedakan dari yang
                            // belum, agar Operator tahu mana yang tinggal
                            // diselesaikan dan mana yang harus dimulai.
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    <div class="progress flex-grow-1" style="height: 6px; min-width: 70px;">
                                        <div class="progress-bar {{ $sudah > 0 ? 'bg-warning' : 'bg-secondary' }}" style="width: {{ $persen }}%"></div>
                                    </div>
                                    <small class="text-muted text-nowrap fw-semibold">{{ $sudah }}/{{ $total }} palet</small>
                                </div>
                            </td>
                            <td class="text-center">
                                <a href="{{ route('wms.inbound.putaway.process', $doc->document_number) }}"
                                   class="btn btn-sm btn-primary rounded-pill px-3 shadow-sm text-nowrap">
                                    {{ $sudah > 0 ? 'Lanjutkan' : 'Proses' }} <i class="bi bi-arrow-right ms-1"></i>
                                </a>
                            </td>
                        </tr>
                    @empty
